...
export async function createSamplePdf(): Promise<Uint8Array> {
  ...
  second.drawText('Project notes', { x: 52, y: 550, size: 22, font: bold, color: rgb(.09, .13, .13) });
  second.drawText('Preferred install: Thursday morning. Keep the existing brass frame.', { x: 52, y: 505, size: 13, font });
  second.drawText('This landscape page lets you test moving, rotating, removing, and restoring pages.', { x: 52, y: 470, size: 12, font, color: rgb(.28, .32, .3) });
  return pdf.save();
}
